...
void splitPos(){
    //min(750,cfmx);
    int cur=K;
    rep(i,cur)if(D[i]>3&&C[i]>0&&!reducted[i]&&abs(C[i]*(D[i]-2))>limit&&bsp.count(i)==0){//bsp.count(i)==0
        int mid=D[i]/2;
...
void splitNeg(){
    //min(750,cfmx);
    int cur=K;
    rep(i,cur)if(D[i]>3&&C[i]<0&&!reducted[i]&&abs(C[i]*(D[i]-1))>limit){
        int mid=D[i]/2;
...
void setoff0(){
    ...
    for(int td=5;td>=3;--td){
        rep(i,K)if(!reducted[i]){
            if(C[i]>0&&D[i]>td){
                ...
                if(id!=-1){
                    bitset<4096> tms=(msk[i]&msk[id]);
                    scf[varCount]+=-C[id]*(D[i]-1);
                    rep(j,V[i].size()){
                        if(tms[V[i][j]]==1){
                            qcf[V[i][j]][varCount]+=C[id];
                        }
                        else{
                            qcf[V[i][j]][varCount]+=-C[id];
                            scf[varCount]+=C[id];
    ...
    for(int td=5;td>=4;--td){
        ...
        /*
        rep(i,K)if(!reducted[i]&&D[i]==td&&C[i]>0){
            rep(j,K)if(!reducted[j]){
                int id=-1;
                if(D[j]==td-1&&C[j]<0&&C[i]+C[j]<=0&&(msk[i]&msk[j])==msk[j]){
                    rep(j2,K)if(C[j2]>0&&!reducted[j2]&&D[j2]>td&&C[j2]+C[i]+C[j]<=0&&(msk[i]&msk[j2])==msk[i]){
                        id=j2;
                        break;
                    }
                    if(id==-1) continue;
                    
                    D[cur]=D[i]+1;C[cur]=-C[id];
                    rep(j2,D[id]){
                        if(msk[i][V[id][j2]]){
                            V[cur].pb(V[id][j2]);
                            H[cur]^=Hash[V[id][j2]];
                    ...
                    int nxv=varCount,nxv2=varCount+1;

                    D[cur]=2;C[cur]=C[j];
                    V[cur].pb(nxv);H[cur]^=Hash[nxv];msk[cur][nxv]=1;
                    V[cur].pb(nxv2);H[cur]^=Hash[nxv2];msk[cur][nxv2]=1;
                    hdic[H[cur]]=cur;
                    ++cur;
                    ...
                    rep(tl,D[i]){
                        if(msk[j][V[i][tl]]==0){
                            D[cur]=2;C[cur]=-C[j];
                            V[cur].pb(V[i][tl]);H[cur]^=Hash[V[i][tl]];msk[cur][V[i][tl]]=1;
                            V[cur].pb(nxv);H[cur]^=Hash[nxv];msk[cur][nxv]=1;
                            hdic[H[cur]]=cur;
                            ++cur;
                        }
...
void setoff2(){
    ...
    rep(i,K)if(!reducted[i]){
        if(C[i]>0&&D[i]>=4){
            ...
            rep(j,cur){
                //min500,mxcf ?
                if(i!=j&&D[j]>=2&&(C[j]+C[i])*(D[j]-1)<=500&&(msk[i]&msk[j])==msk[j]&&mx<D[j]&&!reducted[j]){
                    mx=max(mx,D[j]);
                    id=j;
                }
            }
...
void comsplit(){
    ...
    rep(i,K)if(!reducted[i]&&C[i]>0&&D[i]>=3){
        ...
        while(1){
            bool update=false;
            int nxid=-1,mx=2;
            rep(j,K)if(!reducted[j]&&C[j]>0&&i!=j&&abs(tmpcf+C[j])*(3)<mxcf&&st.count(j)==0){
...
int main(){
    ...
    */
    ...
    /*
    vector<pint> memo;
    rep(i,K)if(!reducted[i]&&C[i]>0&&D[i]==6){
        memo.pb(C[i],i);
    }
    sort(memo.begin(),memo.end());
    rep(i,min(5,(int)memo.size())){
        int id=memo[i].second;
        D[K]=5;
        rep(j,5){
            V[K].pb(V[id][j]);
        }
